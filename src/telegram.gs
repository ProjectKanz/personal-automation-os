...
return;
    ...
    if (msg.text) {
      ...
      if (text === "/share") {
        prepLookerExport();
        sendText(chatId, buildLinkedInCampaignMessage(generateLinkedInCampaign()));
        return;
      }
...
function buildTelegramHelpMessage() {
  return (
    ...
    "14. `/share` - LinkedIn campaign package\n" +
    "15. `/careeradd Company | Role | Status | Notes` - Tambah lamaran\n" +
    "16. `/note habit | alasan` - Tambah catatan habit\n" +
    "17. Ketik nama habit untuk mencentang\n\n" +
    "`/list` tetap bisa dipakai sebagai alias `/help`."
  );
}

function buildLinkedInCampaignMessage(campaign) {
  const metrics = campaign.metrics || {};
  const hookText = campaign.hook
    .map(line => "- " + escapeTelegramMarkdown(line))
    .join("\n");
  const carouselText = campaign.carouselBlueprint
    .map(line => "- " + escapeTelegramMarkdown(line))
    .join("\n");
  const videoText = campaign.videoIdea
    .map(line => "- " + escapeTelegramMarkdown(line))
    .join("\n");

  return (
    "📣 *LinkedIn Campaign Package*\n" +
    "Tone: Management & Marketing | Operational Excellence | AI Engineering\n" +
    "Version Scope: " + escapeTelegramMarkdown(metrics.allowedFeatureScope || VERSION_HISTORY["V3.0-V3.4"]) + "\n\n" +
    "*🧲 Hook:*\n" +
    hookText +
    "\n\n*🖼️ Carousel Blueprint:*\n" +
    carouselText +
    "\n\n*🎬 Video Idea:*\n" +
    videoText +
    "\n\n*📊 Data Signal:*\n" +
    "- Applications analyzed: " + (metrics.totalApplications || 0) + "\n" +
    "- Best CV tag: " + escapeTelegramMarkdown(metrics.bestCvTag || "Not enough data") + "\n" +
    "- Best CV response signal: " + (metrics.bestCvRate || 0) + "%\n" +
    "- CV performance gap: " + formatCvPerformanceGap_(metrics.cvPerformanceGap) + "\n\n" +
    "*📎 Technical Proof:*\n" +
    "- Spreadsheet: " + escapeTelegramMarkdown(metrics.spreadsheetUrl || "Unavailable") + "\n" +
    "- Looker export refreshed: `Looker_Data`\n" +
    "- Copy the GitHub README draft to your repository for deep technical proof.\n\n" +
    "Generated by Kemas Personal Productivity OS " + escapeTelegramMarkdown(metrics.version || SYSTEM_STATE.version)
  );
}

function formatCvPerformanceGap_(gap) {
  if (gap === null || gap === undefined) {
    return "Not enough General CV baseline yet";
  }

  if (gap > 0) {
    return "+" + gap + " pts vs General CVs";
  }
